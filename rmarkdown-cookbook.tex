...
\section{Include Appendix after
Bibliography}\label{include-appendix-after-bibliography}

By default, the bibliography appears at the very end of the document.
However, there can be cases where we want to place additional text after
the references, most typically if we wish to include appendices within
the document. We can force the position of the references by using
...
We can improve this further by using the \textbf{bookdown} package,
which offers a
\href{https://bookdown.org/yihui/bookdown/markdown-extensions-by-bookdown.html\#special-headers}{special
header} for the Appendix as shown with the syntax below:
...
\FunctionTok{date:}\AttributeTok{ !r format(Sys.time(), '%d %B, %Y')}
\OtherTok{---}
\end{Highlighting}
\end{Shaded}

This will automatically update each time you Knit your document i.e.~28
...
\item
  \texttt{\%d/\%m/\%y}: ``28/09/18
\item
  \texttt{\%d/\%m/\%y}: Fri 28 Sep
\end{itemize}
...
\section{Include additional LaTeX
packages}\label{include-additional-latex-packages}

If your primary output format for your R Markdown document is PDF, then
the use of additional LaTeX packages can allow for extensive
customization of document styling. In addition, several packages such as
\href{https://haozhu233.github.io/kableExtra/awesome_table_in_pdf.pdf}{kableExtra}
may have LaTeX dependencies for the R package to function. Much like R,
we need to load packages within the R Markdown document before we are
able to use their functions.

\subsection{Loading LaTeX Packages}\label{loading-latex-packages}

We can load additional LaTeX packages using the
\texttt{extra\_dependencies} option within the \texttt{pdf\_document}
YAML settings. This allows us to provide a list of R Markdown packages:
...
\FunctionTok{output:}\AttributeTok{ }
  \FunctionTok{pdf_document:}
    \FunctionTok{extra_dependencies:}\AttributeTok{ }\KeywordTok{[}\StringTok{"bbm"}\KeywordTok{,} \StringTok{"threeparttable"}\KeywordTok{]}
\OtherTok{---}
\end{Highlighting}
\end{Shaded}

If you need to specify options when loading the package, you can add a
second-level to the list and provide the options as a list:
...
For those familiar with LaTeX, this is equivalent to the following LaTeX
file:
...
packages available through the \href{https://ctan.org/}{Comprohensive
Tex Archive Network} (CTAN). As some examples of LaTeX packages you
could consider using within your analysis:

\begin{itemize}
\tightlist
\item
  \href{https://ctan.org/pkg/pdfpages?lang=en}{pdfpages}: include full
  PDF pages within your document. This is useful if you wish to have
  appendices within your document
\item
  \href{https://ctan.org/pkg/caption?lang=en}{caption}: change the
  appearance of caption subtitles. For example, you can make the Figure
  title italics, centered.
\item
  \href{https://ctan.org/pkg/fancyhdr?lang=en}{fancyhdr}: change the
